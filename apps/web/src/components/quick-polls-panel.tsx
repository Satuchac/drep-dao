  ...
  return (
    <section className="space-y-2 rounded-lg border border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900">
      ...
      <p className="text-xs text-neutral-500">
        §9.2 — proposals with equal scores at the budget cliff. Rank them by priority; the
        remaining category budget is filled top-down, funding as many as fit. 48 h window, 51%
        participation (extended up to 3× when too low).
      </p>
  ...
  return (
    <div className="rounded border border-neutral-200 p-3 text-sm dark:border-neutral-800">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <span className="font-medium">{poll.categoryName ?? 'category'} · tie of {poll.candidates.length}</span>
        <span className="flex items-center gap-2">
          <span className={`rounded px-1.5 py-0.5 text-[11px] font-medium ${STATUS_CLS[poll.status]}`}>{poll.status}</span>
          {poll.status === 'ACTIVE' && poll.endsAt ? <span className="text-xs text-neutral-500">ends {new Date(poll.endsAt).toLocaleString()}{poll.extensions > 0 ? ` · extended ${poll.extensions}×` : ''}</span> : null}
      ...
      <div className="mt-1 text-xs text-neutral-500">
        {poll.votedCount}/{poll.eligibleCount} voted · {poll.remainingAda.toLocaleString()} ₳ budget for this tie
      </div>
      ...
      <ul className="mt-2 space-y-1">
        {poll.candidates.map((c, i) => {
          ...
          const mark = poll.status === 'FAILED' ? { t: '✗ not funded', cls: 'text-red-600' }
            : funded ? { t: '✓ funded', cls: 'text-emerald-600' }
            : c.projectedTie ? { t: '≈ tie — needs a follow-up poll', cls: 'text-amber-600' }
            : { t: poll.status === 'PENDING_BOARD' || poll.status === 'ACTIVE' ? 'projected: not funded' : '✗ not funded', cls: 'text-neutral-400' };
          return (
            <li key={c.id} className={`flex items-center justify-between gap-2 rounded border px-2 py-1.5 text-xs ${funded ? 'border-emerald-300 bg-emerald-50/60 dark:border-emerald-900 dark:bg-emerald-950/30' : 'border-neutral-200 dark:border-neutral-800'}`}>
              ...
              <span className="tabular-nums text-neutral-500" title="Aggregate priority score (power-weighted)">score {c.score.toFixed(2)}</span>
            </li>
          );
        })}
      </ul>
      ...
      {canVote ? (
        <div className="mt-3 rounded-md border border-blue-300 bg-blue-50/50 p-2 dark:border-blue-900 dark:bg-blue-950/20">
          <div className="text-xs font-semibold text-blue-700 dark:text-blue-300">Your priority order {poll.myRanking ? '(submitted — reorder to change)' : '(highest first)'}</div>
              ...
              return (
                <li key={id} className="flex items-center justify-between gap-2 rounded border border-neutral-200 bg-white px-2 py-1 text-xs dark:border-neutral-800 dark:bg-neutral-900">
                  ...
                  <span className="flex items-center gap-1">
                    <button onClick={() => move(i, -1)} disabled={i === 0} className="rounded border border-neutral-300 px-1.5 leading-5 disabled:opacity-30 dark:border-neutral-700" title="Move up">▲</button>
                    <button onClick={() => move(i, 1)} disabled={i === order.length - 1} className="rounded border border-neutral-300 px-1.5 leading-5 disabled:opacity-30 dark:border-neutral-700" title="Move down">▼</button>
                  </span>
                </li>
              );
            })}
          ...
          <button onClick={() => void submit()} disabled={busy} className="mt-2 rounded-md bg-emerald-600 px-3 py-1 text-xs font-medium text-white hover:bg-emerald-700 disabled:opacity-50">
            {busy ? 'Saving…' : poll.myRanking ? 'Update my ranking' : 'Submit my ranking'}
          </button>
        </div>
      ) : null}
    </div>
