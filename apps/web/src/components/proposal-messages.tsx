  ...
  return (
    <div className="rounded-lg border border-neutral-200 p-3 dark:border-neutral-800">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {canBoard && thread.status === 'OPEN' ? (
            <button onClick={markDone} disabled={busy} className="rounded border border-neutral-300 px-2 py-0.5 text-xs hover:bg-neutral-100 disabled:opacity-50 dark:border-neutral-700">Mark done</button>
          ) : null}
        </div>
      </div>
          ...
          <li key={e.id} className={`flex ${e.fromBoard ? 'justify-start' : 'justify-end'}`}>
            <div className={`max-w-[85%] rounded-lg px-3 py-1.5 text-sm ${e.fromBoard ? 'bg-emerald-50 dark:bg-emerald-950/30' : 'bg-sky-50 dark:bg-sky-950/30'}`}>
              <div className="text-[11px] text-neutral-500">
                {e.author ?? (e.fromBoard ? 'Board' : 'Submitter')} · {e.fromBoard ? 'Board' : 'Submitter'} · {new Date(e.createdAt).toLocaleString()}
              </div>
      ...
      {thread.status === 'OPEN' ? (
        ...
        <div className="mt-1 text-xs text-neutral-400">Closed{thread.doneAt ? ` on ${new Date(thread.doneAt).toLocaleDateString()}` : ''}.</div>
      )}
    </div>
  );
}
...
export function ProposalMessagesPanel({ proposalId, canBoard }: { proposalId: string; canBoard: boolean }) {
  ...
  return (
    <section id="proposal-messages" className="space-y-2 rounded-lg border border-neutral-200 p-4 dark:border-neutral-800">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-base font-semibold">Messages {items.length ? `(${items.length})` : ''}</h3>
        {canBoard ? (
      ...
      <p className="text-xs text-neutral-500">Board ↔ submitter messages for this proposal — e.g. asking the team to send the promised tokens to the Treasury. Visible to board members and the submitter only.</p>
      {composing ? (
        <div className="space-y-2 rounded border border-emerald-200 p-2 dark:border-emerald-900">
          <textarea value={draft} onChange={(e) => setDraft(e.target.value)} rows={3} placeholder="Ask the submitter to do something…" className="w-full rounded border border-neutral-300 px-2 py-1 text-sm dark:border-neutral-700 dark:bg-neutral-900" />
          <div className="flex gap-2">
            <button onClick={start} disabled={busy || !draft.trim()} className="rounded bg-emerald-600 px-3 py-1 text-sm text-white hover:bg-emerald-700 disabled:opacity-50">{busy ? 'Sending…' : 'Send'}</button>
            <button onClick={() => { setComposing(false); setDraft(''); }} className="rounded border border-neutral-300 px-3 py-1 text-sm dark:border-neutral-700">Cancel</button>
          </div>
        </div>
      ) : null}
      {items.length === 0 ? <p className="text-sm text-neutral-500">No messages yet.</p> : items.map((t) => <MessageThreadCard key={t.id} thread={t} canBoard={canBoard} onChange={load} />)}
    </section>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-neutral-500">Messages from the board about your proposals — e.g. a request to send tokens to the Treasury. Respond here; the board is notified.</p>
      </div>
      {items.length === 0 ? <p className="text-sm text-neutral-500">No messages.</p> : null}
      <div className="space-y-3">
        {open.map((t) => <MessageThreadCard key={t.id} thread={t} canBoard={false} showProposal onChange={load} />)}
      </div>
      {done.length ? (
        <div className="space-y-2 border-t border-neutral-200 pt-3 dark:border-neutral-800">
          <div className="text-sm font-medium text-neutral-500">Closed ({done.length})</div>
...
export function BoardMessages({ onChange }: { onChange?: () => void }) {
  ...
  return (
    <section className="space-y-2 rounded-lg border border-neutral-200 p-4 dark:border-neutral-800">
      <h3 className="text-base font-semibold">Messages with responses ({threads.length})</h3>
      <p className="text-xs text-neutral-500">§3.5 — submitters replied to these. Respond, or mark the thread done.</p>
