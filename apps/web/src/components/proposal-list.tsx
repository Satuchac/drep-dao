...
/** §26.2 — public list of a round's proposals (DRAFTs are never returned). */
export function ProposalList({ roundId }: { roundId: string }) {
  ...
  return (
    <ul className="space-y-2">
      {proposals.map((p) => (
        <li key={p.id} className="rounded-md border border-neutral-200 px-3 py-2 text-sm dark:border-neutral-800">
