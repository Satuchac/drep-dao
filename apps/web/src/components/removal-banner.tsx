...
export function RemovalBanner() {
  ...
  return (
    <div className="rounded-lg border border-red-300 bg-red-50 p-4 text-sm dark:border-red-900 dark:bg-red-950/40">
      <h3 className="font-semibold text-red-700 dark:text-red-300">⚠ Removal vote in progress</h3>
      <p className="mt-1 text-neutral-700 dark:text-neutral-300">
        {r.proposedByName} proposed removing you from the DAO — <strong>{r.yes}/{r.threshold}</strong> YES so far.
        {r.reason ? ` Reason: ${r.reason}` : ''} If {r.threshold} board members vote YES you lose membership (you can
        re-apply afterwards).
      </p>
      {r.votes.length ? (
