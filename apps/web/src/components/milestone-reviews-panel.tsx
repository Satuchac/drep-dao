 ...
 */
export function MilestoneReviewsPanel() {
  const { setParams } = useUrlNav();
  const [items, setItems] = useState<MilestoneAssignmentView[]>([]);
  const load = useCallback(() => {
    milestonesApi.myAssignments().then(setItems).catch(() => setItems([]));
  }, []);
  useEffect(load, [load]);
  ...
  return (
    <section className="space-y-2 rounded-lg border border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900">
      <h3 className="text-base font-semibold">Milestone reviews — your assignments ({items.length})</h3>
